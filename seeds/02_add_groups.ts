...
export async function seed(knex: Knex): Promise<void> {
  ...
  for (let i = 0; i < floorCount; ++i) {
    for (let j = 0; j < groupsPerFloor; ++j) {
      ...
      console.log(
        `Connect: owner #${connectOwner.userId} to #${connectOwner.groupId}`
      )
  ...
  console.log(
    '\x1b[32m%s\x1b[0m',
    `Inserted ${groupsPerFloor * floorCount} groups into db.`
  )
  console.log(
    '\x1b[32m%s\x1b[0m',
    `Inserted ${connectCountSum} users_groups into db.`
  )
}
