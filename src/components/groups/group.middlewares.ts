...
export const joinGroup = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (group.doNotDisturb && user.id !== group.ownerId) {
      ...
      await Group.relatedQuery('users').for(group.id).relate(user.id)
      return next()
    }
...
export const leaveGroup = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction) => {
...
export const isMemberInGroup = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction) => {
    const kickableUser = await Group.relatedQuery('users')
      .for(req.group.id)
...
export const isGroupOwnerOrAdmin = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction) => {
    if (
      (req.user as User)?.id === req.group.ownerId ||
...
export const validateGroup = (): ValidationChain[] => {
  return [
    check(
      'name',
      'A csoport neve max 100 karakter hosszú nem üres szöveg lehet'
    )
      .isString()
    ...
    check('tags')
      ...
      .custom((value: string) =>
        value.split(',').every((it) => it.length <= 30)
      )
    ...
    check()
      ...
      .custom(
        (value) =>
          value.type !== 'floor' || !(value.room < 3 || value.room > 18)
      )
    ...
    check('startDate')
      ...
      .custom(
        (value, { req }) =>
          new Date(value).getTime() < new Date(req.body.endDate).getTime()
      )
      .withMessage('A kezdés nem lehet korábban, mint a befejezés')
      .custom(
        (value, { req }) =>
          differenceInMinutes(new Date(req.body.endDate), new Date(value)) <=
          5 * 60
      )
      .withMessage('A foglalás időtartama nem lehet hosszabb 5 óránál'),
    check('endDate', 'A befejezés időpontja kötelező').exists({
      checkFalsy: true,
      checkNull: true,
    }),
...
export const checkValidMaxAttendeeLimit = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction) => {
    if (req.group.users.length > (req.body.maxAttendees || 100)) {
      res.status(400).json({
        errors: [
          { msg: 'Nem lehet kisebb a maximum jelenlét, mint a jelenlegi' },
        ],
      })
    } else {
      next()
