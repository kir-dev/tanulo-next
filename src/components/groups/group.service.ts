...
export const getGroup = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (group) {
      ...
      if (/\/copy|\/edit/.test(req.path)) req.group = group
      else
        req.group = {
          ...group,
          description: formatMdToSafeHTML(group.description),
        } as Group
      next()
    } else {
...
export const createGroup = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction) => {
    req.group = await Group.query().insert({
      name: req.body.name,
      tags: req.body.tags ?? '',
...
export const removeGroup = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction) => {
    await Group.transaction(async (trx) => {
      await Group.relatedQuery('users', trx).for(req.group.id).unrelate()
