...
export const createTicket = asyncWrapper(
  async (req: Request, _res: Response, next: NextFunction) => {
    await Ticket.transaction(async (trx) => {
      return await Ticket.query(trx).insert({
...
export const sendEmailToTicketAdmins = asyncWrapper(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const emailRecepients = await User.query().where({
      role: RoleType.TICKET_ADMIN,
    })
...
export const moveTicket = asyncWrapper(
  async (req: Request, _res: Response, next: NextFunction) => {
    ...
    await Ticket.query()
      .findById(id)
      .patch({ status: req.body.status || StatusType.SENT })
    next()
  }
)
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!isNaN(id)) {
      ...
      if (deletedCount === 0) {
        res
          .status(404)
          .send({ message: 'Nem található hibajegy a megadott ID-val' })
      } else {
        next()
      }
    } else {
